...
class MapperApp:
    ...
    def update_mapping_label(self):
        if os.path.exists(MAPPING_FILE):
            ...
            self.lbl_mapping.config(text='(Nenhum)')
    ...
    def save_hotkeys(self):
        try:
            ...
            print("Erro ao salvar hotkeys:", e)
    ...
    def set_hotkey(self, which):
        ...
        try:
            ...
            self.save_hotkeys()
            self.register_hotkeys()
            messagebox.showinfo('Ok', f'Hotkey definida: {key}')
        except Exception as e:
            messagebox.showerror('Erro', f'Erro ao definir hotkey: {e}')

    def define_trigger_key(self):
        key = self.capture_single_key_modal('Defina a tecla trigger (uma execução)')
        if not key:
            return
        try:
            try: keyboard.remove_hotkey(self.trigger_key)
            ...
            self.save_hotkeys()
            self.register_hotkeys()
            messagebox.showinfo('Ok', f'Trigger definida: {key}')
        except Exception as e:
            messagebox.showerror('Erro', f'Erro ao definir trigger: {e}')
    ...
    def capture_single_key_modal(self, title):
        ...
        def finalize(key):
            lbl.config(text=f'Tecla detectada: {key}')
